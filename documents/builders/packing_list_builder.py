"""
Packing List Context Builder
============================
Builder kontekstu dla dokumentow listy pakunkowej.
"""
...
class PackingListContextBuilder(BaseContextBuilder):
    """
    Builder kontekstu dla dokumentu Listy Pakunkowej.

    Zawiera:
    - Pełną liste detali z thumbnailami
    - Informacje o wadze i wymiarach
    - Grupowanie po materialach
    ...
    def build(
        ...
        """
        Zbuduj kontekst dla listy pakunkowej.
        ...
        # Sortuj po materiale i grubosci
        sorted_items = sorted(
            items,
            key=lambda x: (
                x.get('material', '') or '',
        ...
        # 5. Informacje o opakowaniach
        packages_info = self._get_packages_info(order)
        ...
        extra_data = {
            ...
            'packaging_weight': order.get('packaging_weight', 0),
            'packages': packages_info,
            'shipping_address': self._get_shipping_address(order),
            'special_handling': order.get('special_handling', ''),
            'fragile': order.get('fragile', False),
            'packing_date': date.today().strftime('%Y-%m-%d'),
            'packed_by': user_id
        }
        ...
        return DocumentContext(
            ...
            doc_type_label=DOCUMENT_LABELS[DocumentType.PACKING_LIST],
            ...
            buyer=buyer,
            items=doc_items,
            # Lista pakunkowa bez cen
            total_net=None,
            total_vat=None,
            total_gross=None,
            currency=order.get('currency', 'PLN'),
            notes=order.get('packing_notes', order.get('notes', '')),
            footer_text='Towar sprawdzono przed pakowaniem.',
    ...
    def _get_packages_info(self, order: dict) -> dict:
        """Pobierz informacje o opakowaniach"""
        return {
            'pallets': order.get('pallets_count', 0),
            'boxes': order.get('boxes_count', 0),
            'dimensions': order.get('package_dimensions', ''),
            'type': order.get('package_type', 'paleta')
        }
